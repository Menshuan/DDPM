...
class Unet(nn.Module):
    def __init__(
        ...
        # 如果clip_dim存在，则将其加入到input_channel
        if clip_dim is not None:
            input_channels += clip_dim
    ...
    def forward(self, x, time, x_self_cond=None, clip_emb=None):
        if self.self_condition:  # 自我条件化的话，将对应张量合并。
            x_self_cond = default(x_self_cond, lambda: torch.zeros_like(x))
            x = torch.cat((x_self_cond, x), dim=1)
        # 如果clip_emb存在，将其调整尺寸后与输入x拼接
        if clip_emb is not None:
            clip_emb = clip_emb.unsqueeze(-1).unsqueeze(-1)
            clip_emb = clip_emb.expand(-1, -1, x.shape[2], x.shape[3])
        ...
        for block1, block2, attn, upsample in self.ups:
            # print(f"Before concatenation: {x.shape}")
